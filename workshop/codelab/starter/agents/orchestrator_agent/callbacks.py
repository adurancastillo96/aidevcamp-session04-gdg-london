    ...
    try:
        ...
        if execution_key in _agent_execution_tracker:
            ...
            if total_execution_time > 20:
                logger.warning("Slow agent: %s took %.2fs",
                               agent_name, total_execution_time)
        ...
        if content.role == "user":
            for part in (content.parts or []):
                ...
                for topic in BLOCKED_TOPICS:
                    if topic in text:
                        print(
                            f"  🛡️ GUARDRAIL: Blocked request containing '{topic}'")
                        return LlmResponse(
